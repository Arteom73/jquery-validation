(function () {
	...
	var additionalMethods = createCommonjsModule(function (module) {
	/*!
	 * jQuery Validation Plugin v1.20.0
	 *
	 * https://jqueryvalidation.org/
	...
	*/
	...
	jquery.fn.form = function (data) {
	    if (data === 'destroy') {
			jquery(this).data('validator').destroy();
	    } else {
			...
			$module.find('[data-rule]').each(function (index, item) {
				if (rules[jquery(item).data('rule')]) {
					jquery(item).rules('add', rules[jquery(item).data('rule')]);
				}
			});
